...
docker compose up -d --build  # installs the PREBUILT OpenClaw (no source compile)
...
> **No compile step → builds on tiny VMs.** The image installs the published
> `openclaw` npm package (already built) instead of compiling from source, so
> there is no `tsdown` step to OOM. `npm install` is light; even a 1 GB-RAM VPS
> handles it (a couple GB of swap helps the install go smoothly). The app only
> uses the OpenAI-compatible `/v1/chat/completions` endpoint, which the stock
> package provides — no custom build needed.
>
> Pin a different OpenClaw version via the build arg if needed:
> ```bash
> docker compose build --build-arg OPENCLAW_VERSION=2026.6.1
> ```
> Note: running the gateway + Codex on ~1 GB RAM is workable for low/occasional
> use (reports, proposals) but leans on swap; 2 GB is comfortable.

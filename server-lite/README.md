...
`codex exec` once (no tools, ephemeral), and returns the assistant reply in
OpenAI format. Your reports/proposals already parse JSON out of the text reply,
so nothing in the app changes — just point `OPENCLAW_URL` at this.
...
| Features | full agent platform | chat completion only |

...
export default function AIInsightsCard({ clientId, clientName, range, hasMetrics, metrics = [], canManage = false }: Props) {
  ...
  const generate = async () => {
    ...
    try {
      ...
      // Dev override: route the report to a locally-served function (OpenClaw-wired)
      // when VITE_LOCAL_FUNCTIONS_URL is set; otherwise use the deployed function.
      const localFnUrl = import.meta.env.VITE_LOCAL_FUNCTIONS_URL as string | undefined;
      let resp: any;
      let error: any = null;
      if (localFnUrl) {
        try {
          const r = await fetch(`${localFnUrl}/metrics-insights`, {
            method: 'POST',
            headers: {
        ...
        ({ data: resp, error } = await supabase.functions.invoke('metrics-insights', {
          body: reqBody,
            ...
            <div ref={previewRef} className="bg-card p-8 space-y-8">
              ...
              <section>
                    ...
                    {data.kpi_highlights.map((k, i) => {
                      ...
                      return (
                        <div key={i} className="rounded-xl border border-border bg-secondary/40 p-4">
                          ...
                          <div className="mt-2 font-display text-2xl font-bold text-accent tabular-nums leading-none">{(k.value || '').replace(/\*\*/g, '')}</div>
                          <div className={cn('mt-2 flex items-center gap-1 text-xs font-semibold', trendColor)}>
                            <TrendIcon className="h-3 w-3" /> {(k.trend_pct || '').replace(/\*\*/g, '')}
                          </div>
                        </div>
                      );
                    })}
                ...
                <p className="mt-3 text-[11px] text-muted-foreground italic">
                  Source: aggregated metrics from connected platforms (see Section 2). KPI selection, trend % and narrative generated by {(data as any)?.engine || AI_MODEL_LABEL}.
                </p>
              </section>
                        ...
                        <div key={prov} className="space-y-3">
                          ...
                          {breakdowns.length > 0 ? (
                            <>
                              ...
                              <p className="text-[11px] text-muted-foreground italic">
                                Analysis generated by {(data as any)?.engine || AI_MODEL_LABEL}, based on {meta.label} data above. Interpretations are AI-assisted and should be reviewed by a Leadzap strategist before action.
                              </p>
                            </>
                          ) : (
              ...
              <section>
                ...
                <p className="mt-3 text-[11px] text-muted-foreground italic">
                  Wins, concerns and recommendations synthesized by {(data as any)?.engine || AI_MODEL_LABEL} from the data above. Final strategic decisions are made by the Leadzap team.
                </p>
              </section>
              ...
              <footer className="mt-4 rounded-2xl border border-border bg-secondary/30 p-6 space-y-4">
                ...
                <div className="border-t border-border pt-4 space-y-2 text-[11px] text-muted-foreground leading-relaxed">
                  <p><strong className="text-foreground/80">Disclaimer.</strong> This report is prepared by Leadzap Marketing for the exclusive use of {clientName || 'the client'}. Quantitative data is sourced directly from the third-party platforms listed under each chart. Narrative analysis, KPI selection and recommendations are generated by {(data as any)?.engine || AI_MODEL_LABEL} and reviewed by Leadzap Management. Sections labelled "Info by Leadzap Management" reflect human-curated commentary and supporting visuals provided by the Leadzap team.</p>

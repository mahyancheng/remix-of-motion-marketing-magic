...
const BudgetCalculator = () => {
  ...
  const [budgetRatio, setBudgetRatio] = useState(20); // 15-25%
        ...
        <div className="mx-auto max-w-5xl">
          <div className="grid gap-8 lg:grid-cols-2">
            ...
            <div className="rounded-2xl border border-primary-foreground/10 bg-primary-foreground/5 p-6 backdrop-blur md:p-8">
              ...
              <div className="space-y-8">
                ...
                <div className="space-y-3">
                  ...
                  <Slider
                    value={[targetRevenue]}
                    onValueChange={(value) => setTargetRevenue(value[0])}
                    min={10000}
                    max={1000000}
                    step={10000}
                    className="w-full"
                ...
                <div className="space-y-3">
                  ...
                  <Slider
                    value={[budgetRatio]}
                    onValueChange={(value) => setBudgetRatio(value[0])}
                    min={15}
                    max={25}
                    step={1}
                    className="w-full"
            ...
            <div className="space-y-6">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="rounded-xl border border-primary-foreground/10 bg-primary-foreground/5 p-5">
                  <div className="mb-2 flex items-center gap-2 text-primary-foreground/60">
                    ...
                    <span className="text-sm">Conversions Needed</span>
                  </div>
                  <div className="font-display text-3xl font-bold text-primary-foreground">
                    {results.conversionsNeeded.toLocaleString()}
                  </div>
                ...
                <div className="rounded-xl border border-primary-foreground/10 bg-primary-foreground/5 p-5">
                  <div className="mb-2 flex items-center gap-2 text-primary-foreground/60">
                    ...
                    <span className="text-sm">Leads Needed</span>
                  </div>
                  <div className="font-display text-3xl font-bold text-primary-foreground">
                    {results.leadsNeeded.toLocaleString()}
                  </div>
                  <div className="mt-1 text-xs text-primary-foreground/50">
                    Enquiries per month
                  </div>
              ...
              {/* Formula breakdown */}
              <div className="rounded-xl border border-primary-foreground/10 bg-primary-foreground/5 p-5">
                <h4 className="mb-3 font-semibold text-primary-foreground">
                  How it's calculated:
                </h4>
                <div className="space-y-2 text-sm text-primary-foreground/70">
                  <p>
                    <span className="text-primary-foreground">Conversions:</span>{" "}
                    {formatCurrency(targetRevenue)} ÷ {formatCurrency(averageOrderValue)} ={" "}
                    {results.conversionsNeeded}
                  </p>
                  <p>
                    <span className="text-primary-foreground">Leads:</span>{" "}
                    {results.conversionsNeeded} ÷ {conversionRate}% ={" "}
                    {results.leadsNeeded}
                  </p>
                  <p>
                    <span className="text-primary-foreground">Budget:</span>{" "}
                    {formatCurrency(targetRevenue)} × {budgetRatio}% ={" "}
                    {formatCurrency(results.marketingBudget)}
                  </p>
                  <p>
                    <span className="text-primary-foreground">Estimated CPL:</span>{" "}
                    {formatCurrency(results.marketingBudget)} ÷ {results.leadsNeeded} ={" "}
